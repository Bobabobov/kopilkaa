  ...
  return (
    ...
    >
      ...
      {friendship && friendship.status === "ACCEPTED" && (
        <div className="w-full text-center px-4 py-3 rounded-2xl text-sm font-semibold bg-[#10B981]/20 text-[#10B981]">✓ Друзья</div>
      )}

      {friendship && friendship.status === "DECLINED" && (
        <div className="w-full text-center px-4 py-3 rounded-2xl text-sm font-semibold bg-red-500/20 text-red-400">Заявка отклонена</div>
      )}
    </motion.div>
  );
}
