...
interface ApplicationsListProps {
  ...
  onStatusChange: (id: string, status: ApplicationItem["status"], comment?: string) => void;
  onQuickUpdate: (id: string, status: ApplicationItem["status"], comment?: string) => void;
  ...
  return (
    <div className="grid gap-6">
      {items.map((item, index) => {
        const preview = item.story.length > 260 ? item.story.slice(0, 260) + "…" : item.story;
            ...
            <div className="relative">
              ...
              <div className="flex flex-wrap items-start justify-between gap-4 mb-6">
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    <div className="text-sm text-gray-500 dark:text-gray-400 break-words">
                      ...
                      {shownEmails.has(item.id) ? (
                        <span className="ml-1">{!item.user.hideEmail ? item.user.email : 'Email скрыт'}</span>
                      ) : (
                        <button
                ...
                <div className="flex items-center gap-3 shrink-0">
                  ...
                  <button
                    ...
                    onClick={() => onQuickUpdate(item.id, "APPROVED", item.adminComment || "")}
                  >
                    Одобрить
                  </button>
                  <button
                    ...
                    onClick={() => onQuickUpdate(item.id, "REJECTED", item.adminComment || "")}
                  >
                    Отказать
                  </button>
                  <button
              ...
              <details className="toggle text-sm mb-6">
                ...
                <div className="open-only rounded-2xl bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 border border-blue-200 dark:border-blue-800 mt-3 relative group">
                  <div className="px-4 py-3 break-words pr-16">
                    <span className="text-blue-600 dark:text-blue-400 font-medium">Реквизиты: </span>
                    ...
                    onClick={(e) => {
                      ...
                      navigator.clipboard.writeText(item.payment).then(() => {
                        ...
                        if (icon && text) {
                          ...
                          btn.className = btn.className.replace("hover:bg-blue-100", "bg-green-100").replace("hover:text-blue-700", "text-green-700");
                          setTimeout(() => {
                            icon.textContent = "📋";
                            text.textContent = "Копировать";
                            btn.className = btn.className.replace("bg-green-100", "hover:bg-blue-100").replace("text-green-700", "hover:text-blue-700");
                          }, 1500);
                        }
                      }).catch(() => {
                  ...
                  {item.images.map((img, i) => (
                    <div key={i} className="group relative overflow-hidden rounded-2xl">
                      <img
                        ...
                        onClick={() => onImageClick(item.images.map(x => x.url), i)}
                      />
              ...
              {item.adminComment && (
                <div className="text-sm rounded-xl bg-black/5 dark:bg-white/10 px-3 py-2 break-words mt-4">
                  <span className="opacity-70">Комментарий модератора: </span>{item.adminComment}
                </div>
              )}
            </div>
          </motion.div>
