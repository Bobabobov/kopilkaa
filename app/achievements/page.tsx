...
export default function AchievementsPage() {
  ...
  const raritySummaryCards = Object.entries(rarityCounts).map(
    ([key, value]) => {
      const k = key as AchievementRarity;
      const percent = value.total
        ? Math.round((value.unlocked / value.total) * 100)
        : 0;
  ...
  const filtered = useMemo(() => {
    return progress
      .filter((p) => {
        ...
        const text =
          `${p.achievement.name} ${p.achievement.description}`.toLowerCase();
  ...
  const refreshProgress = async () => {
    try {
      ...
      const check = await fetch("/api/achievements/user/check", {
        method: "POST",
      });
      if (check.status === 401) {
      ...
      <div className="relative z-10 container mx-auto px-4 py-10 space-y-8">
          ...
          <div className="flex flex-col xl:flex-row xl:items-center xl:justify-between gap-6">
            <div className="space-y-3 max-w-3xl w-full">
              ...
              <p className="text-[#abd1c6] leading-relaxed">
                Витрина всех ачивок платформы. Карточки только для просмотра,
                без кликов. Фильтруй по редкости и статусу, ищи по названию или
                описанию.
              </p>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 w-full">
              {[
                ...
                >
                  ...
                  <div className="text-xl font-semibold text-[#fffffe]">
                    {item.value}
                  </div>
                  {item.hint && (
                    <div className="text-[10px] text-[#94a1b2]/80 mt-1">
                      {item.hint}
                    </div>
                  )}
                </div>
            ...
            <div className="h-3 rounded-full overflow-hidden border border-white/10 bg-white/10">
              <div
                ...
                style={{
                  width: `${completion}%`,
                  background:
                    "linear-gradient(90deg, #f9bc60 0%, #f9d18c 100%)",
                  ...
                  <div className="flex items-center justify-between text-[11px] text-[#c7d4d0]">
                    <span className="inline-flex items-center gap-1">
                      <span
                        ...
                        style={{ backgroundColor: RARITY_COLORS[item.key] }}
                      />
        ...
        <div className="rounded-[24px] border border-white/10 bg-white/5/60 backdrop-blur-xl p-4 sm:p-5 shadow-[0_18px_48px_rgba(0,0,0,0.28)] space-y-4">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-sm text-[#abd1c6]">
            <div className="flex items-center gap-2 flex-wrap">
              <span>
                Показано {filteredCount} из {total}
              </span>
              {activeFilters > 0 && (
            ...
            <span className="text-[#94a1b2]">
              Отфильтруйте, чтобы сузить список
            </span>
          </div>
        </div>
        ...
        {unauthorized ? (
          ...
          <div className="rounded-[30px] border border-white/10 bg-white/5/60 backdrop-blur-xl p-4 sm:p-6 space-y-4 shadow-[0_24px_60px_rgba(0,0,0,0.35)]">
            ...
            {filtered.length === 0 && (
              <div className="text-center text-[#abd1c6] py-6">
