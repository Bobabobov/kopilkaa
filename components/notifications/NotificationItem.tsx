...
export default function NotificationItem({
  ...
  const avatarSize = isDropdown
    ? "w-8 h-8 sm:w-9 sm:h-9"
    : "w-10 h-10 sm:w-12 sm:h-12";
  const iconSize = isDropdown
    ? "w-4 h-4 sm:w-4.5 sm:h-4.5"
    : "w-5 h-5 sm:w-6 sm:h-6";
  const titleSize = isDropdown ? "text-xs sm:text-sm" : "text-sm sm:text-base";
  const messageSize = isDropdown
    ? "text-[10px] sm:text-xs"
    : "text-xs sm:text-sm";
  const borderClass = isUnread
    ? isDropdown
      ? "border-l-2 border-l-[#f9bc60]"
      : "border-l-4 border-l-[#f9bc60]"
    : "";
  ...
  return (
    ...
    >
      ...
      <div
        className={`flex items-start ${isDropdown ? "gap-2 sm:gap-2.5" : "gap-3 sm:gap-4"}`}
      >
        {/* Аватар или иконка */}
        <div
          className={`${avatarSize} rounded-lg overflow-hidden bg-[#004643] flex items-center justify-center flex-shrink-0 border border-[#abd1c6]/20`}
        >
          {notification.avatar ? (
            <img
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-1.5 sm:gap-2 mb-0.5 sm:mb-1">
            <h4
              className={`font-medium text-[#fffffe] ${titleSize} break-words leading-tight ${
            ...
            <span
              className={`text-[9px] sm:text-[10px] text-[#abd1c6]/50 flex-shrink-0 ${isDropdown ? "ml-1.5" : ""} whitespace-nowrap`}
            >
              {notification.timestamp}
            </span>
          </div>
          <p
            className={`${messageSize} text-[#abd1c6]/80 ${isDropdown ? "line-clamp-2" : "line-clamp-2"} break-words leading-snug ${isDropdown ? "" : "mb-2"}`}
          >
