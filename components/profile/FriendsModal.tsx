...
export default function FriendsModal({ isOpen, onClose, initialTab = 'friends' }: FriendsModalProps) {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      if (event.key === 'Escape') {
        ...
        const tabs: Array<'friends' | 'sent' | 'received' | 'search'> = ['friends', 'sent', 'received', 'search'];
  ...
  const tabs = [
    { id: 'friends' as const, label: 'Друзья', count: friends.length, icon: '👥' },
    { id: 'sent' as const, label: 'Отправленные', count: sentRequests.length, icon: '📤' },
    { id: 'received' as const, label: 'Полученные', count: receivedRequests.length, icon: '📥' },
