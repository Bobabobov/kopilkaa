  ...
  try {
    ...
    if (withLinks && userIds.length > 0) {
      ...
      for (const row of globalApps) {
        addFp(
          row.paymentFingerprint ?? digitsFingerprint(row.payment),
          row.userId,
        );
      ...
      type PageApp = (typeof appsOnPage)[number];
      type PageWd = (typeof withdrawalsOnPage)[number];
      const userApps = new Map<string, PageApp[]>();
      for (const app of appsOnPage) {
        const list = userApps.get(app.userId) ?? [];
        list.push(app);
        userApps.set(app.userId, list);
      }
      const userWithdrawals = new Map<string, PageWd[]>();
      for (const w of withdrawalsOnPage) {
      ...
      for (const user of usersWithTrust) {
        ...
        for (const w of userWithdrawals.get(user.id) ?? []) {
          const fp = w.detailsFingerprint ?? digitsFingerprint(w.details);
          if (fp) myFingerprints.add(fp);
        }
        ...
        for (const fp of myFingerprints) {
          ...
          if (uids) {
            for (const uid of uids) {
              if (uid !== user.id) samePaymentIds.add(uid);
            }
          }
        }
        for (const ip of myIps) {
          ...
          if (uids) {
            for (const uid of uids) {
              if (uid !== user.id) sameIpIds.add(uid);
            }
          }
        }
