...
- Используйте **`sanitizeEmailForViewer(user, viewerId)`** из `@/lib/privacy` везде, где в ответе возвращаются данные другого пользователя (профили, списки друзей, истории, баг-репорты и т.п.).
