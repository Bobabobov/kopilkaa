  ...
  const calculated = useMemo<PersonalStatsViewModel | null>(() => {
    ...
    const successHint = (() => {
      if (totalApplications === 0) {
        return "Опубликуйте первую историю и проверьте, как быстро её увидит редакция.";
      }
      if (totalApplications < 3) {
        return stats.applications.approved === totalApplications
          ? "Все ваши истории одобрены — отличный старт!"
          : "Пока мало данных, но каждая подробная история повышает шанс на одобрение.";
      }
      if (approvedPercent >= 80) {
        return "Отличный результат: большинство ваших историй одобряют. Продолжайте в том же духе!";
      }
      ...
      return "Почти все истории отклоняются. Проверьте правила, уточните сумму гонорара и покажите, на что планируете потратить вознаграждение.";
    })();
