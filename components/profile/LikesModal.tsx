...
export default function LikesModal({ isOpen, onClose }: LikesModalProps) {
  ...
  const getUserStatus = (lastSeen: string | null) => {
    ...
    return { status: "offline", text: lastSeenDate.toLocaleDateString('ru-RU') };
  };
        ...
        >
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 rounded-2xl backdrop-blur-xl flex items-center justify-center" style={{ backgroundColor: 'rgba(0, 30, 29, 0.2)' }}>
                  <svg className="w-6 h-6" style={{ color: '#001e1d' }} fill="currentColor" viewBox="0 0 24 24">
                    <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/>
                  </svg>
                </div>
                <div>
                  <h2 className="text-2xl font-bold" style={{ color: '#001e1d' }}>Лайки ваших историй</h2>
          ...
          <AnimatePresence>
            {localNotification.show && (
              ...
              >
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ backgroundColor: 'rgba(0, 30, 29, 0.2)' }}>
          ...
          <div className="p-4 max-h-[60vh] overflow-y-auto">
            {loading ? (
              ...
              <div className="text-center py-12">
                ...
                <h3 className="text-xl font-bold mb-3" style={{ color: '#fffffe' }}>Пока нет лайков</h3>
              ...
              <div className="space-y-4">
                {likes.map((like, index) => {
                  ...
                  return (
                  <motion.div
                    key={like.id && like.id.trim() ? like.id : `modal-like-${index}`}
                    initial={{ opacity: 0, y: 16 }}
                    animate={{ opacity: 1, y: 0 }}
                    transition={{ delay: 0.02 * index, duration: 0.5, ease: [0.16, 1, 0.3, 1] }}
                  ...
                  >
                    <div className="flex items-start gap-4">
                      {/* Аватар */}
                      <div className="relative w-12 h-12 rounded-xl overflow-hidden flex items-center justify-center text-white font-semibold text-sm flex-shrink-0 shadow-lg" style={{ backgroundColor: '#f9bc60' }}>
                        {like.user.avatar ? (
                          <img
                          ...
                          <span>
                            {(like.user.name || like.user.email.split('@')[0])[0].toUpperCase()}
                          </span>
                        )}
                        ...
                        style={{ backgroundColor: userStatus.status === 'online' ? '#f9bc60' : '#abd1c6' }}
                        title={userStatus.status === 'online' ? 'Онлайн' : `Последний вход: ${userStatus.text}`}></div>
                      </div>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between mb-3">
                          ...
                          <div className="text-sm ml-4" style={{ color: '#abd1c6' }}>
                            <span>{new Date(like.createdAt).toLocaleDateString('ru-RU')}</span>
                          </div>
                        </div>

                        {/* История */}
                        <Link href={`/stories/${like.application.id}`} className="block">
                          <div className="rounded-xl p-3 hover:shadow-lg transition-all"
                            style={{ 
                              backgroundColor: 'rgba(249, 188, 96, 0.1)',
                              border: '1px solid rgba(249, 188, 96, 0.2)'
                            }}>
                            <h4 className="font-semibold mb-2 line-clamp-1" style={{ color: '#fffffe' }}>
                              {like.application.title}
                            </h4>
                            <p className="text-sm line-clamp-2 mb-2" style={{ color: '#abd1c6' }}>
                              {like.application.summary}
                            </p>
                            <div className="flex items-center gap-1 text-sm font-medium" style={{ color: '#f9bc60' }}>
