...
export default function AdSection({ variant = "sidebar" }: AdSectionProps) {
  ...
  if (variant === "feed") {
    ...
    const mobileImageUrl = (ad?.config?.sidebarMobileImageUrl && ad.config.sidebarMobileImageUrl.trim() !== "")
      ...
      : displayAd.imageUrl;
    const mobileTitle = (ad?.config?.sidebarMobileTitle && ad.config.sidebarMobileTitle.trim() !== "")
      ? ad.config.sidebarMobileTitle
      : (displayAd.title || "Реклама");
    const mobileContent = (ad?.config?.sidebarMobileContent && ad.config.sidebarMobileContent.trim() !== "")
  ...
  return (
    <div className="w-full">
      ...
      >
        <div className={`relative rounded-3xl border border-[#abd1c6]/30 bg-[#001e1d]/90 shadow-2xl shadow-black/40 overflow-hidden transition-all duration-300 ${
          displayAd.linkUrl ? 'cursor-pointer hover:border-[#f9bc60]/50 hover:shadow-[#f9bc60]/20' : ''
        }`} style={{ height: '260px' }}>
          ...
          <div className="relative z-10 h-full flex flex-col md:flex-row items-center justify-between gap-4 px-5 md:px-8">
            ...
            <div className="flex flex-col items-stretch md:items-end gap-2 text-right">
              <div className={`inline-flex items-center justify-center rounded-lg px-5 py-2 text-xs md:text-sm font-semibold transition-colors ${
                displayAd.linkUrl
                  ? "bg-[#f9bc60] text-[#001e1d]"
                  : "bg-transparent text-[#abd1c6]/80 border border-[#abd1c6]/60"
              }`}>
                {displayAd.linkUrl ? "Перейти" : "Скоро здесь может быть ваша реклама"}
              </div>
            </div>

            {/* Контакт в правом нижнем углу */}
            <div className="absolute right-5 bottom-3 text-[11px] text-[#abd1c6]/85" onClick={(e) => e.stopPropagation()}>
              Написать:{" "}
              <a
                href="mailto:support@kopilka-online.ru"
