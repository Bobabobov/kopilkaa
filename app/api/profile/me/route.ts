...
export async function PATCH(req: Request) {
  try {
    ...
    if (name !== undefined && (typeof name !== 'string' || name.length > 100)) {
      return Response.json({ error: "Имя должно быть строкой не более 100 символов" }, { status: 400 });
    }
    
    if (email !== undefined) {
      if (typeof email !== 'string' || !email.includes('@') || email.length > 255) {
        return Response.json({ error: "Некорректный email адрес" }, { status: 400 });
      }
      ...
      if (existingUser) {
        return Response.json({ error: "Этот email уже используется другим пользователем" }, { status: 400 });
      }
    }
    
    if (hideEmail !== undefined && typeof hideEmail !== 'boolean') {
      return Response.json({ error: "hideEmail должно быть булевым значением" }, { status: 400 });
    }
