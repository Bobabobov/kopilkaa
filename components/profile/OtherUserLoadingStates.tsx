...
export default function OtherUserLoadingStates({ state }: OtherUserLoadingStatesProps) {
  const getContent = () => {
    switch (state) {
      case 'checking':
        return {
      ...
      case 'unauthorized':
        return {
          ...
          description: 'Войдите в аккаунт или зарегистрируйтесь, чтобы просмотреть профили',
          buttons: [
            {
              href: '/login',
  ...
  return (
    <div className="min-h-screen flex items-center justify-center">
      ...
      >
        ...
        {content.description && (
          <p className="text-gray-600 dark:text-gray-400 mb-8 text-lg">{content.description}</p>
        )}
        {content.button && (
