...
export function AchievementCard({
  ...
  const progressPercentage = effectiveTarget > 0 ? (effectiveCurrent / effectiveTarget) * 100 : 0;
  ...
  const borderColor = isUnlocked ? currentTheme.border : 'rgba(255,255,255,0.14)';
  const lockedOverlay = !isUnlocked ? 'after:content-[\" \"] after:absolute after:inset-0 after:bg-[#040808]/45 after:backdrop-blur-[1px] after:pointer-events-none' : '';
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none">
        ...
        <div className="absolute inset-0 opacity-30" style={{ background: `radial-gradient(120% 120% at 10% 20%, ${RARITY_COLORS[achievement.rarity]}25 0%, transparent 45%)` }} />
      </div>
      ...
      <div className="flex flex-col gap-5 relative z-10">
        ...
        <div
          ...
          style={{
                ...
                boxShadow: isUnlocked ? `0 16px 30px ${RARITY_COLORS[achievement.rarity]}35` : `0 12px 22px rgba(0,0,0,0.28)`,
        ...
        >
              <div
                ...
                style={{ background: `radial-gradient(circle at 30% 30%, ${RARITY_COLORS[achievement.rarity]}60 0%, transparent 55%)` }}
              />
              <IconComponent size="lg" className="relative" />
        </div>
            <div className="space-y-1 min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                <h3 className={`text-lg font-semibold leading-tight break-words ${isUnlocked ? 'text-[#fdfdfd]' : 'text-[#a7b8b3]'}`}>
              {achievement.name}
            </h3>
                {isUnlocked ? (
                  <span className="text-[#abd1c6]"><LucideIcons.CheckCircle size="sm" /></span>
                ) : (
                  <span className="text-[#92a3a0]"><LucideIcons.Lock size="sm" /></span>
            )}
          </div>
              <p className={`text-sm leading-relaxed break-words ${isUnlocked ? 'text-[#d9e8e2]' : 'text-[#9fb2ac] opacity-95'} line-clamp-3`}>
          ...
          <div className="flex flex-wrap items-center gap-2 md:justify-end self-start md:self-end">
            ...
            >
              <span className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: RARITY_COLORS[achievement.rarity] }} />
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between text-xs text-[#b3c7c1] flex-wrap gap-2">
              ...
              <span className="font-semibold text-[#e7f3ee] flex items-center gap-2">
                ...
                {effectiveCurrent}/{effectiveTarget} ({Math.round(progressPercentage)}%)
              </span>
          </div>
