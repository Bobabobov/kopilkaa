...
export default function AdSection({ variant = "sidebar" }: AdSectionProps) {
  ...
  // Вариант для горизонтального баннера (используется сейчас под кнопками на главной)
  return (
    <div className="w-full">
      <div className="relative rounded-3xl border border-[#abd1c6]/30 bg-[#001e1d]/90 shadow-2xl shadow-black/40 overflow-hidden h-40 md:h-48">
          ...
          <div className="flex flex-col items-stretch md:items-end gap-2 text-right">
            <a
              ...
              className={`inline-flex items-center justify-center rounded-lg px-5 py-2 text-xs md:text-sm font-semibold transition-colors ${
                displayAd.linkUrl
                  ? "bg-[#f9bc60] text-[#001e1d] hover:bg-[#e8a545]"
                  : "bg-transparent text-[#abd1c6]/80 border border-[#abd1c6]/60 cursor-default"
              }`}
            >
