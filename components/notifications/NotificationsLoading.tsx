...
export default function NotificationsLoading({ variant = "feed" }: NotificationsLoadingProps) {
  ...
  return (
    <div className={`${paddingClass} text-center`}>
      ...
      >
        <LucideIcons.Loader2 className={`text-[#f9bc60] mx-auto ${isDropdown ? 'mb-2' : 'mb-4'}`} size={iconSize} />
      </motion.div>
      <p className={`${isDropdown ? 'text-xs sm:text-sm' : 'text-sm'} text-[#abd1c6]`}>
