...
export default function AdminApplicationPage({ params }: { params: { id: string } }) {
  ...
  const deleteApplication = async () => {
    try {
      const r = await fetch(`/api/admin/applications/${item?.id}`, { method: "DELETE" });
      if (r.ok) {
      ...
      showToast("error", "Ошибка удаления", "Произошла ошибка при удалении заявки");
    }
  };
          ...
          <div className="relative space-y-6">
            ...
            <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-gray-900/50 rounded-xl p-4">
              ...
              <button
                onClick={() => {
                  ...
                  navigator.clipboard.writeText(email).then(() => {
                    showToast("success", "Email скопирован!", "Адрес автора добавлен в буфер обмена");
                  }).catch(() => {
              ...
              >
                ...
                <svg className="w-3 h-3 text-emerald-500 group-hover:scale-110 transition-transform duration-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <details className="toggle">
              ...
              <div className="open-only rounded-2xl bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 relative group">
                <div className="px-6 py-4 break-all text-anywhere pr-16">
                  <span className="text-emerald-700 dark:text-emerald-300 font-medium">Реквизиты: </span>
                  <span className="select-all text-gray-900 dark:text-white">{item.payment}</span>
                </div>
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    navigator.clipboard.writeText(item.payment).then(() => {
                      ...
                      if (icon && text) {
                        ...
                        btn.className = btn.className.replace("hover:bg-emerald-100", "bg-green-100").replace("hover:text-emerald-700", "text-green-700");
                        setTimeout(() => {
                          icon.textContent = "📋";
                          text.textContent = "Копировать";
                          btn.className = btn.className.replace("bg-green-100", "hover:bg-emerald-100").replace("text-green-700", "hover:text-emerald-700");
                        }, 1500);
                      }
                    }).catch(() => {
              ...
              {item.images.length > 0 ? (
                ...
                <div className="text-center py-8 text-gray-500 dark:text-gray-400">
                  ...
                  <p className="text-lg font-medium">Фотографии не прикреплены</p>
            ...
            {item.adminComment && (
              <div className="rounded-2xl bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 p-6">
                ...
                <div className="break-all text-anywhere text-gray-800 dark:text-gray-200">{item.adminComment}</div>
              </div>
            )}
                        ...
                        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-6 border-t border-gray-200 dark:border-gray-700">
                          <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
                            ...
                            <span>Отправлено: {new Date(item.createdAt).toLocaleString()}</span>
                          </div>
