...
export function useApplicationFormState() {
  ...
  const trustHint = `Доступная сумма для вашего уровня: до ${trustLimits.max.toLocaleString("ru-RU")} ₽`;
  ...
  const exceedsTrustLimit =
    approvedCount !== null && Number.isFinite(amountInt) && amountInt > trustLimits.max;
  ...
  const handleAmountInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const clampedDigits =
      nextDigits.length === 0
        ? ""
        : Math.min(parseInt(nextDigits, 10), trustLimits.max).toString();
